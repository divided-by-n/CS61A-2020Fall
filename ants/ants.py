...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        count = 0
        while check_place.name != 'Hive':
            if check_place.bees and self.min_range <= count <= self.max_range: 
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount > 0:
            [a.reduce_armor(amount) for a in bees]
        else:
            [a.reduce_armor(amount + self.damage) for a in bees]
