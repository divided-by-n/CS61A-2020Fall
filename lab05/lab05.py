...
def closer_city(lat, lon, city_a, city_b):
    ...
    d_a=distance(city_c,city_a)
    d_b=distance(city_c,city_b)
    return get_name(city_b) if d_a>d_b else get_name(city_a)
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[int(i/2)] if i%2==0 else deck[int((len(deck)+i-1)/2)] for i in range(0,len(deck))]
...
def add_trees(t1, t2):
    ...
    if t2 is None:
        ...
        return tree(label(t1) + label(t2), [add_trees(x, y) for x, y in itertools.zip_longest(branches(t1), branches(t2))])
...
def height(t):
    ...
    if is_leaf(t):
        ...
        for b in branches(t):
            return height(tree(label(b),branches(b)))
